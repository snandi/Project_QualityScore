...
#################################################################################
## get the conversion factor for each molecule
def conversion_factor(ChrNum, moleID, GroupNum, MoleculeNum, content):

	regex0 = 'chr' + str(ChrNum) + '\s[0-9]+\s[0-9]+\s' + moleID + '\s[0-9]+\s[0-9]+\s[0-9]+\s[0-9]+\s([0-9]*)\s([0-9]*)\s.+?\s.*'
	all = map(list, re.findall(regex0, content))
	for i in range(0, len(all)):
		all[i] = map(int, all[i])
	all = np.array(all)
	bottom = np.min(all)
	numbps = np.max(all) - np.min(all)
	...
	fileContents = mfc.read_molecule_files(molecule_file_whole)
	numPixels = mfc.get_numPixels(fileContents)

	convert = int(math.trunc(numbps / numPixels))+1
	
	return convert, bottom

#################################################################################
## convert base pair coordinates to pixel indices
def convert_bp2pixel(bp_coordinate, convert):
	bp_per_pixel = convert #* math.pow(LengthRatio, 1)
	if(bp_coordinate == 0):
		pixel_coordinate = 0
	else:
		pixel_coordinate = int(math.trunc((bp_coordinate-1) / bp_per_pixel))
	
	return pixel_coordinate
	
#################################################################################
## generate the molecule file containing certain region(aligned)
def get_region_file(Input_moleParentFolder, Output_moleParentFolder, IntervalNum, GroupNum, MoleculeNum,start_pixelind, end_pixelind, orientation):
	...
	molecule_file_whole = os.path.join(Input_moleParentFolder, GroupName, MoleculeName)
	molecule_file_region = os.path.join(Output_moleParentFolder, IntervalName, GroupName, MoleculeName)
...
#	print  start_pixelind, end_pixelind, numPixels_whole
	if(start_pixelind>numPixels_whole or end_pixelind>numPixels_whole):
#		print "out of range!"
#		print  start_pixelind, end_pixelind, numPixels_whole
	...
	Output_GroupFolder = os.path.join(Output_moleParentFolder, IntervalName, GroupName)
...
#	ChrNum = int(argv[1])
	...
	for mole in mole_list:
		...
		MoleculeNum = int(moleID[15:19])
		
		
		convert, bottom = conversion_factor(ChrNum, moleID, GroupNum, MoleculeNum, content)
		...
		if(len(s) > 1):
			...
			region = list(s[0])
			start_bp = int(region[0])-bottom
			end_bp = int(region[1])-bottom
			orientation = int(region[2])
			LengthRatio = float(region[3])

			start_pixelind = convert_bp2pixel(start_bp, convert)
			end_pixelind = convert_bp2pixel(end_bp, convert)
			a = get_region_file(Input_moleParentFolder, Output_moleParentFolder, IntervalNum, GroupNum, MoleculeNum,start_pixelind, end_pixelind, orientation)
			
			if(a < 0): 
				print moleID, "  discrepancy:  " ,-a, "  LengthRation: ", LengthRatio
				k += 1


	print k, "out of range"
	print b, "bad molecules"
