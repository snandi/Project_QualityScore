...
fn_ClusterOutputs_mm52 <- function(
  Chr, 
  FragIndex, 
  DataPath.mm52
){
  ...
  MoleculeIDs_fromIntensity <- try(unique(as.vector(IntensityData_inRange$MoleculeID)))
...
foreach(FragIndex = Frags, .inorder=FALSE, .packages=Packages_Par) %dopar% fn_ClusterOutputs_mm52(Chr = Chr, FragIndex, DataPath.mm52)
stopCluster(cl)
gc()
